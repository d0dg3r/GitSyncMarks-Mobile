...
3. **Update F-Droid metadata** (in a separate commit, before or after the tag):
   - In [com.d0dg3r.gitsyncmarks-fdroid-submit.yml](metadata/com.d0dg3r.gitsyncmarks-fdroid-submit.yml): add build block with `commit:` (full hash from `git rev-parse vX.Y.Z`), `versionName`, `versionCode`
   - Builds must be chronological (oldest first). No comments in YAML (rewritemeta fails).
...
5. **Commit hash:** New builds use `COMMIT_PLACEHOLDER`. After committing, replace and amend, then tag:
   ```bash
   HASH=$(git rev-parse HEAD)
   sed -i "s/COMMIT_PLACEHOLDER/$HASH/" fdroid/metadata/com.d0dg3r.gitsyncmarks-fdroid-submit.yml
   git add fdroid/metadata/com.d0dg3r.gitsyncmarks-fdroid-submit.yml && git commit --amend --no-edit
   git tag vX.Y.Z
   ```
